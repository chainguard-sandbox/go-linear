...
func NewStatusUpdateArchiveCommand(clientFactory cli.ClientFactory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `⚠️ Archive initiative status update. Cannot be undone. Prompts unless --yes.
...
Related: initiative_status-update-list, initiative_status-update-get`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runStatusUpdateArchive(cmd, client, args[0], confirmFlags, outputFlags)
		},
	}
...
func runStatusUpdateArchive(cmd *cobra.Command, client *linear.Client, updateID string, confirmFlags *cli.ConfirmationFlags, outputFlags *cli.OutputOnlyFlags) error {
	ctx := cmd.Context()
	...
	// Confirmation
	if !confirmFlags.Yes {
		fmt.Fprintf(cmd.OutOrStderr(), "⚠️  Archive initiative status update %s? This cannot be undone.\n", updateID)
