...
// FilterBuilder constructs UserFilter from CLI flags.
...
// Filter returns the underlying UserFilter.
func (b *FilterBuilder) Filter() *intgraphql.UserFilter {
	return b.filter
}
...
func (b *FilterBuilder) Build() *intgraphql.UserFilter {
	if !hasAnyFilter(
		b.filter.ID,
		b.filter.Name,
		b.filter.Email,
		b.filter.DisplayName,
		b.filter.Active,
		b.filter.Admin,
		b.filter.IsMe,
		b.filter.CreatedAt,
