...
func TestLive_ProjectFilterBuilder(t *testing.T) {
	...
	t.Run("date range filter", func(t *testing.T) {
		...
		first := int64(50)
		projects, err := client.ProjectsFiltered(ctx, &first, nil, filter)
		if err != nil {
			t.Fatalf("ProjectsFiltered() error = %v", err)
		}

		t.Logf("Found %d projects created in last 90 days", len(projects.Nodes))
	})
	...
	t.Run("name filter", func(t *testing.T) {
		...
		t.Logf("Found %d projects with 'a' in name", len(projects.Nodes))
	})
	...
	t.Run("combined filters", func(t *testing.T) {
		...
		t.Logf("Found %d projects from last 180 days", len(projects.Nodes))
	})
}
