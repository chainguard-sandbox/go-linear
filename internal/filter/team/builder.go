...
// FilterBuilder constructs TeamFilter from CLI flags.
type FilterBuilder struct {
	filter   *intgraphql.TeamFilter
...
// Filter returns the underlying TeamFilter.
func (b *FilterBuilder) Filter() *intgraphql.TeamFilter {
	return b.filter
}
...
func (b *FilterBuilder) Build() *intgraphql.TeamFilter {
	if !hasAnyFilter(
		b.filter.ID,
		b.filter.Name,
		b.filter.Key,
